...
export const ChatRoomScreen: React.FC = () => {
   ...
   * - Documents → messageType "audio" (reusing the closest available backend enum value).
   *   If you want a dedicated "document" type, add it to the MessageType enum in both
   *   models/conversation.ts and types/backendType.ts, then change "audio" → "document" here.
   */
  const handleUploadAndSend = async (
    ...
    try {
      ...
      const newMessage = await sendMessage(
        conversation._id,
        uploaded.fileName, // content = file name (caption or document label)
        backendMessageType,
        uploaded.url, // mediaUrl
      );
  ...
  const renderMessage = ({ item }: { item: IMessage }) => {
    ...
    // documents are stored with messageType "audio" — see handleUploadAndSend
    const isDoc = item.messageType === "audio" && !!item.mediaUrl;
  ...
  return (
    <SafeAreaView style={styles.screen} edges={["top"]}>
      ...
      {/* ── Messages + Input (KeyboardAvoidingView wraps both so keyboard pushes them up together) ── */}
      <KeyboardAvoidingView
        style={styles.flex}
      ...
      >
        ...
        {/* ── Attachment menu (inline, above input bar — no position:absolute) ── */}
        {showAttachMenu && (
...
const styles = StyleSheet.create({
  ...
  // Inline attach menu — sits between FlatList and input bar in normal flow
  attachMenu: {
    flexDirection: "row",
    gap: 12,
    backgroundColor: "#fff",
